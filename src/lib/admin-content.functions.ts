...
const newsSchema = z.object({
  ...
  category: z.enum(["evento","feria","taller","curso_nuevo","producto_nuevo","historia","inspiracion","promocion"]),
...
export const adminUpsertNews = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await context.supabase.from("news_posts").upsert(payload, { onConflict: "id" }).select("id").single();
    if (error) throw error;
    return row;
  });
...
export const adminListWorkshops = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    await assertStaff(context);
    const { data, error } = await context.supabase.from("workshops").select("*").order("starts_at", { ascending: true });
    if (error) throw error;
    return data ?? [];
  });
...
export const adminUpsertWorkshop = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload: any = { ...data };
    if (payload.cover_image_url === "") payload.cover_image_url = null;
    const { data: row, error } = await context.supabase.from("workshops").upsert(payload, { onConflict: "id" }).select("id").single();
    if (error) throw error;
    return row;
  });
...
export const adminListEnrollments = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = context.supabase
      ...
      .select("id, full_name, email, phone, payment_status, amount, notes, created_at, workshop:workshops(id, title, slug, starts_at)")
...
export const adminUpdateEnrollmentPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("workshop_enrollments").update(payload).eq("id", data.id);
    if (error) throw error;
    return { ok: true };
  });
...
export const adminDeleteEnrollment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertStaff(context);
    const { error } = await context.supabase.from("workshop_enrollments").delete().eq("id", data.id);
    if (error) throw error;
    return { ok: true };
  });
...
export const adminListFairs = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await context.supabase
      .from("fairs")
      .select("*, warehouse:warehouses(code, name), items:fair_items(id, qty_sent, qty_sold, qty_returned, product:products(name, sku))")
...
export const adminUpsertFair = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertStaff(context);
    const { data: row, error } = await context.supabase.from("fairs").upsert(data, { onConflict: "id" }).select("id").single();
    if (error) throw error;
    return row;
  });
...
export const adminUpsertFairItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertStaff(context);
    const { data: row, error } = await context.supabase.from("fair_items").upsert(data, { onConflict: "id" }).select("id").single();
    if (error) throw error;
    return row;
  });
...
export const adminReports = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [salesMonth, salesDay, lowStock, topItems, paymentsByMethod, leadsCount, customersCount] = await Promise.all([
      sb.from("sales").select("total, status").gte("created_at", startMonth).eq("status","confirmada"),
      sb.from("sales").select("total, status").gte("created_at", startDay).eq("status","confirmada"),
      sb.from("inventory_stock").select("quantity, product:products(id, name, sku, min_stock), warehouse:warehouses(code, name)").limit(500),
    ...
    const lowStockList = (lowStock.data ?? []).filter((r: any) => r.product?.min_stock != null && Number(r.quantity) <= Number(r.product.min_stock))
      .slice(0, 50);

    const map = new Map<string, { name: string; qty: number; revenue: number }>();
    for (const it of (topItems.data ?? []) as any[]) {
      const id = it.product?.id; if (!id) continue;
      const cur = map.get(id) ?? { name: it.product.name, qty: 0, revenue: 0 };
      cur.qty += Number(it.quantity); cur.revenue += Number(it.subtotal);
      map.set(id, cur);
    }
    const top = [...map.values()].sort((a,b) => b.qty - a.qty).slice(0, 10);

    const byMethod: Record<string, number> = {};
    for (const p of (paymentsByMethod.data ?? []) as any[]) byMethod[p.method] = (byMethod[p.method] ?? 0) + Number(p.amount);

    return {
      sales: { today: sumDay, month: sumMonth, todayCount: salesDay.data?.length ?? 0, monthCount: salesMonth.data?.length ?? 0 },
...
export const clientGetProfile = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const sb = context.supabase;
    const [{ data: profile }, { data: customer }, { data: enrollments }, { data: sales }] = await Promise.all([
      sb.from("profiles").select("*").eq("id", context.userId).maybeSingle(),
      sb.from("customers").select("*").eq("user_id", context.userId).maybeSingle(),
      sb.from("workshop_enrollments").select("*, workshop:workshops(*)").eq("user_id", context.userId).order("created_at", { ascending: false }),
      sb.from("sales").select("*, items:sale_items(*, product:products(name)), payments:sale_payments(*), receipt:receipts(id, number)")
        .order("created_at", { ascending: false }),
    ]);
...
export const clientUpdateProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const sb = context.supabase;
    await sb.from("profiles").update({ full_name: data.full_name, phone: data.phone || null }).eq("id", context.userId);
