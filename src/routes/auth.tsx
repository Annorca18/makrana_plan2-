  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      <div className="hidden lg:flex flex-col justify-between bg-terracotta text-warm-white p-12">
        <Link to="/" className="font-display text-2xl">Makrana Home Art</Link>
        <div>
          <h1 className="font-display text-5xl leading-tight">Hecho a mano, pensado para tu hogar.</h1>
          <p className="mt-4 opacity-90">Accede a tu cuenta para ver tus pedidos, comprobantes y talleres.</p>
        </div>
        <p className="text-sm opacity-70">© {new Date().getFullYear()} Makrana Home Art</p>
      </div>
      <div className="flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          <Link to="/" className="text-sm text-accent">← Volver al inicio</Link>
          <h2 className="font-display text-3xl mt-4">{mode === "login" ? "Ingresa a tu cuenta" : "Crea tu cuenta"}</h2>
          <p className="text-sm text-muted-foreground mt-1">
            {mode === "login" ? "Acceso a la plataforma Makrana." : "Únete a la comunidad Makrana."}
          </p>
          <form onSubmit={onSubmit} className="mt-6 grid gap-4">
            {mode === "signup" && (
              <div className="grid gap-2"><Label htmlFor="full_name">Nombre completo</Label><Input id="full_name" name="full_name" required maxLength={120} /></div>
            )}
            <div className="grid gap-2"><Label htmlFor="email">Correo</Label><Input id="email" name="email" type="email" required maxLength={160} /></div>
            <div className="grid gap-2"><Label htmlFor="password">Contraseña</Label><Input id="password" name="password" type="password" required minLength={8} /></div>
            <Button type="submit" variant="hero" size="lg" disabled={loading}>{loading ? "..." : mode === "login" ? "Ingresar" : "Crear cuenta"}</Button>
          </form>
          <button onClick={() => setMode(mode === "login" ? "signup" : "login")} className="mt-6 text-sm text-accent">
