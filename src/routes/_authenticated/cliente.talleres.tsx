...
function Talleres() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="text-muted-foreground text-sm mt-1">Todas tus inscripciones a talleres y cursos Makrana.</p>
      ...
      {enrolls.length === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed border-sand p-12 text-center bg-cream/40">
          ...
          <Button asChild variant="hero" className="mt-4"><Link to="/talleres">Ver talleres disponibles</Link></Button>
        </div>
      ) : (
            ...
            <li key={e.id} className="border border-sand/60 rounded-xl bg-warm-white p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{formatDate(e.workshop?.starts_at)} · {e.workshop?.modality} · {e.workshop?.level}</div>
                  {e.workshop?.location && <div className="text-xs text-muted-foreground">📍 {e.workshop.location}</div>}
                </div>
                <div className="text-right space-y-1">
                  <Badge variant={e.payment_status === "pagado" ? "default" : "outline"}>pago: {e.payment_status}</Badge>
                  <div className="text-sm tabular-nums">{moneyPEN(e.amount || e.workshop?.price)}</div>
                </div>
              </div>
            </li>
          ))}
